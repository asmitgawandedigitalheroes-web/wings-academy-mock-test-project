...
import { getStats, getCategories, getSubjectsWithCategories, deleteSubject, toggleSubjectStatus } from '@/app/actions/admin'
...
export default function SubjectManager() {
  ...
  const handleToggleStatus = async (subject: any) => {
    const res = await toggleSubjectStatus(subject.id, subject.status || 'enabled')
    if (res.success) {
      fetchData()
    } else {
      alert(res.error || 'Failed to update subject status')
    }
  }
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 relative">
          ...
          {subjects.map((subject: any) => (
            <div key={subject.id} className="group relative">
              <Link 
                  ...
                  className={`block bg-white p-6 rounded-[2rem] border border-slate-100 shadow-lg shadow-primary/5 hover:border-primary/20 transition-all relative overflow-hidden h-full ${subject.status === 'disabled' ? 'opacity-60 grayscale-[0.5]' : ''}`}
              >
                  ...
                  <div>
                     <h3 className="text-lg font-black text-[#0f172a] mb-2 capitalize">{subject.name}</h3>
                  </div>
              ...
              <div className="absolute top-6 right-6 z-20" ref={activeMenu === subject.id ? menuRef : null}>
                  <button 
                      onClick={(e) => {
                          e.preventDefault()
                          e.stopPropagation()
                  ...
                  <AnimatePresence>
                    {activeMenu === subject.id && (
                        ...
                        >
                            <Link 
                                href={`/admin/subjects/${subject.id}/settings`}
      ...
      <ConfirmationModal 
          ...
          message="This will permanently delete this subject and all associated tests and questions. This action cannot be undone."
          type="danger"

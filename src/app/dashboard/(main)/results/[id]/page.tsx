  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
      ...
      <div className={`p-10 md:p-14 rounded-[3rem] border shadow-2xl flex flex-col items-center text-center ${
          ...
          <h1 className="text-4xl font-black text-[#0f172a] mb-2">Test {result.status}!</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
              { label: 'Correct', value: result.correct_answers, icon: CheckCircle2, color: 'text-accent bg-accent/5' },
              { label: 'Incorrect', value: result.total_questions - result.correct_answers, icon: XCircle, color: 'text-primary bg-primary/5' },
      ...
      <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-2xl shadow-primary/5 flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="flex items-center gap-4">
              ...
              <div>
                  ...
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">{result.subjectName}</p>
              </div>
          </div>
